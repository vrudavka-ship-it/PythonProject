...
def _stream_supervisor(query: str, thread_id: str) -> bool:
    ...
    Один thread_id = одна безперервна розмова з можливістю resume.
    """
...
def main() -> None:
    ...
    Кожен запит користувача — окремий thread_id.
    Це означає: кожне дослідження — незалежний stateful workflow.
    thread_id зберігає стан між interrupt (HITL) і resume.

    uuid4() — генератор унікальних ідентифікаторів, як UUID.randomUUID() у Java.
    """
    print("Multiagent Research System (homework-lesson-8)")
